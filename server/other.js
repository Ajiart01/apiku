...
axios.get(`https://api.countapi.xyz/hit/hint.api.akuari.my.id${date1}${date2}${date3}`);
...
router.get('/attp3', async(req, res) => {
  var text = req.query.text
  if (!text) return res.json({ 'message': 'masukan parameter text!'})
    const data = await getBuffer(`https://atp-arii.herokuapp.com/api/maker/attp?text=${text}`)
    res.set({'Content-Type': 'gif'})
res.send(data)
  await apivisit()
})
...
router.get('/attp2', async(req, res) => {
  var text = req.query.text
  if (!text) return res.json({ 'message': 'masukan parameter text!'})
  const kdfl = await attp(text)
    const data = await getBuffer(kdfl.result)
     res.set({'Content-Type': 'gif'})
res.send(data)
  await apivisit()
})
...
router.get('/short/tiny', async (req, res) => {
